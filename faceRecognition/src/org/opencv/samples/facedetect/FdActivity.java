...
public class FdActivity extends Activity implements CvCameraViewListener {
            ...
            switch (status) {
                case LoaderCallbackInterface.SUCCESS:
                {
                    ...
                    try {
                    	...
                    	//(HomeWork4) Here we load in our face image, process it to grayscale, and create 2 image matrices
                    	myFace = Utils.loadResource(myAc, R.drawable.myface);
                    	Imgproc.cvtColor(myFace, myFace, Imgproc.COLOR_RGBA2GRAY);
	...
	//here you get the current frame on the camera
    public Mat onCameraFrame(Mat inputFrame) {
    
    
        inputFrame.copyTo(mRgba);
        
        //convert our input fram into a grayscale image
        ...
        //(HomeWorkChanges): ADD CODE BELOW FOR FACE RECOGNITION (Steps 1-5). Use the documentation here: http://docs.opencv.org/java/ and elsewhere online
        //here we take the detected faces in the image and turn it into an array. We will be checking each of the faces in the image to check if it is OUR face
        Rect[] facesArray = faces.toArray();
        for (int i = 0; i < facesArray.length; i++){
        	
        	//put a green box around the face we've detected in the current frame
